...
import { getCompletedMedicines } from '../../utils/storage';
import GradientCard from '../../components/GradientCard';

const CompletedMedicines = ({ navigation }) => {
  ...
  const loadMedicines = async () => {
    const data = await getCompletedMedicines();
    setMedicines(data);
  };
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {medicines.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyText}>No completed medicines yet</Text>
          </View>
        ) : (
          medicines.map((item, index) => {
            const timeObj = new Date(item.takenAt);
            return (
              <GradientCard key={index} colors={['#F0FDF4', '#DCFCE7']} style={styles.card}>
...
export default CompletedMedicines;
